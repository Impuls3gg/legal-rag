...
своём сервере. Пользователь задаёт вопрос, система находит
...
Основные кодексы скачиваются с ru.wikisource.org одной командой: ГК (все
четыре части), УК, ТК и КоАП. Тексты с метаданными складываются в `data/raw/`.
Скачивание занимает несколько минут.

```bash
docker compose run --rm web python scripts/fetch_codes.py
```

Редакции на Викитеке отстают от действующих на несколько лет: последние
поправки в УК датированы 2019 годом, в ГК ч. 4 — 2022-м. Для реальных
ответов сверяйтесь с [pravo.gov.ru](http://pravo.gov.ru).

Отдельный кодекс или диапазон глав скачивается через
`scripts/fetch_wikisource.py` (параметры — в `--help`). Свой текст добавляется через
...
```bash
docker compose run --rm web python scripts/evaluate.py        # eval/codes.jsonl
docker compose run --rm web python scripts/evaluate.py -v     # результат по каждому вопросу
```

Набор `eval/codes.jsonl` — 98 вопросов с эталонными статьями по всем кодексам
в базе и 9 вопросов вне базы (СК, НК, ЖК, ГПК, УПК, защита прав
потребителей). Статья определяется парой «акт + номер», так как номера
повторяются между кодексами. Скрипт считает Recall@k и MRR на уровне статей,
отдельно по каждому акту, и строит таблицу порогов отказа.
...
scripts/
  fetch_codes.py       загрузка основных кодексов (ГК, УК, ТК, КоАП) с Викитеки
  fetch_wikisource.py  загрузка одного кодекса с Викитеки по главам
  add_act.py           интерактивное добавление акта из буфера обмена
  evaluate.py          метрики поиска на eval-наборе
eval/codes.jsonl     вопросы с эталонными статьями
